...
#' @description This function is designed to plot prettier versions of one dimensional smooths created by \code{\link[mgcv]{plot.gam}}. For each smooth, the necessary information for plotting is taken from a user-supplied list generated by \code{\link[mgcv]{plot.gam}}. Plots are created using \code{\link[prettyGraphics]{pretty_plot}}. Confidence intervals can be added via an internal call to \code{\link[prettyGraphics]{add_model_predictions}}. Partial residuals and a rug for the observed values can also be added and customised.
...
#' @param fit_args A named list of arguments to customise the appearance of the fitted line. This is passed to \code{\link[graphics]{lines}} or the \code{CI_gp} argument of \code{\link[prettyGraphics]{add_model_predictions}}.
...
#' @param se_type A character that defines the method by which CIs are added to the plot: as lines (\code{"lines"}) or as a shaded polygon (\code{"poly"}). This is passed to the \code{fCI} argument of \code{\link[prettyGraphics]{add_model_predictions}}.
#' @param se_args A named list of arguments to customise the appearance of the confidence intervals. This is passed to the \code{CI_gp} argument of \code{\link[prettyGraphics]{add_model_predictions}}.
...
pretty_smooth_1d <- function(fit = list(),
  ...
  int <- loop(1:length(select), function(i){
    ...
    if(!add_se){
      ...
      add_model_predictions_args$x <- ifit$x
      add_model_predictions_args$CI <- cis
      add_model_predictions_args$fCI <- se_type
      add_model_predictions_args$CI_gp <- se_args
      add_model_predictions_args$add_fitted <- TRUE
      if(length(fit_args) == 0) fit_args$col <- "black"
      add_model_predictions_args$fitted_gp <- fit_args
