---
title: "Vignette: An introduction to plot.pretty"
...
vignette: >
  %\VignetteIndexEntry{introducing_plot_pretty}
...
`plot.pretty` is an R package designed to make the production of plots and data exploration easier, more flexible and prettier, with base R. The creation of this package was inspired by the development of another package, Tools4ETS, which provides tools for ecological timeseries in R. Consequently, `plot.pretty` has been particularly motivated by the requirements of datasets with continuous responses and explanatory variables (e.g., depth and time, in relation to covariates). The package is based on a number of functions which act as 'building blocks'; these help to define the initial arguments of a plot and then add elements to a plot sequentially. The package includes some integrative functions, which link these functions together to produce pretty plots in a variety of cases (e.g. a more flexible implementation of `graphics::plot()` with pretty axes, prettier histograms, density plots, timeseries, model residuals etc.). Notable functionality of `plot.pretty` includes the following: 
...
* **Model predictions**. The addition of model predictions (e.g. fitted values ± confidence intervals) on top of observations is critical step in the evaluation of models (e.g. to understand the extent to which they explain observed patterns). This is facilitated by `list_CIs()` and `add_model_predictions()`. 
* **Integrative functions**. `plot.pretty` provides several functions which use the power of building block functions to create prettier plots, including histograms, density plots, residual plots and a number of others. For example, `pretty_plot()` functions like `graphics::plot()` but uses `pretty_axis()` to define pretty axes and is more flexible. `pretty_plot()` can incorporate multiple plotting functions (not just `graphics::plot()`) but some functions (e.g. `graphics::hist()`) necessitate a specialised plotting approach which `plot.pretty` may provide (e.g., `pretty_hist()`). 
...
First, install and load `plot.pretty`:
```{r}
...
Although we will use these data as an example, it is important to emphasise that`plot.pretty`'s functions work for many continuous response and explanatory covariates; they are not restricted to timeseries (though they are particularly useful for some timeseries data.) A few functions are also designed to work with discrete data. 

# `plot.pretty` building blocks 
...
#### Use Tools4ETS to define the positions of day/night periods 
data_shade <-
  Tools4ETS::define_time_blocks(t1 = min(df$timestamp),
                                t2 = max(df$timestamp + 24*60*60),
                                type = "diel",
                                type_args = list(lon = 56.41535, lat = -5.47184),
                                to_plot = TRUE,
                                col = c("white", scales::alpha("grey", 0.5))
                                )
...
During or following data exploration, models are useful tools to elucidate trends and relationships. To evaluate the performance of models, it is often useful to plot model predictions ontop of observed data. Two functions make this easy with `plot.pretty`: `list_CIs()` and `add_model_predictions()`. To illustrate this, below I consider a model of depth as a smooth function of time. Prior to modelling, it is helpful to 'flag' and then 'thin' the timeseries using functions from `Tools4ETS` to reduce serial autocorrelation. Then, I model the thinned data as a function of time, and add predictions to the plot. 

```{r, fig.width = 7, fig.height = 5}
#### Flag and thin timeseries to reduce serial autocorrelation 
# See Tools4ETS for more details on these functions. 
df <- cbind(df, Tools4ETS::flag_ts(df$timestamp, duration_threshold = 720))
dat_flapper_thin <- Tools4ETS::thin_ts(df, flag1 = "flag1", first = 1, nth = 10)

#### Implement a preliminary model of depth ~ s(time) using mgcv::bam and the thinned dataset 
m1 <- mgcv::bam(depth ~ s(scale(as.numeric(timestamp), scale = FALSE), bs = "cr", k = 10), rho = 0.83, data = dat_flapper_thin)
...
`pretty.plot` contains a few integrative functions which implement some of the building blocks described above for particular purposes, with more or less generality. Current functions include `plot_residuals()`, which produces a raft of pretty diagnostic residual plots for a model, `pretty_hist()` and `pretty_density()` which produce histograms and density plots (which can be useful in data exploration and model diagnostics) and `pretty_plot()` which is very like `graphics::plot()` but uses the power of `pretty_axis()` to make the axes and can handle several function types (e.g. `stats::qqnorm()`, see below). 

## Model residuals: `plot_residuals()`

To examine the extent to which model assumptions are met, residual diagnostic plots are instructive. These include standard diagnostic plots (i.e., residuals versus fitted values, residuals versus the linear predictor, a histogram of residuals and a quantile-quantile plot of residuals) as well as plots of residuals against covariates (`vars`, below) and, for timeseries, timestamps and autocorrelation functions (ACF) of residuals. This functionality is provided by `plot_residuals()`, which plots standard diagnostic plots with pretty axes as well as residuals against one or more variables, timestamps and the ACF (if applicable). 

```{r, fig.width = 7, fig.height = 10}
pp <- par(mfrow = c(4, 2), oma = c(2, 2, 1, 1), mar = c(3, 3, 3, 3))
plot_residuals(residuals = m1$std.rsd, 
               fv = stats::fitted(m1),
               lp = stats::fitted(m1), 
               vars = "temp",
               timestamp = "timestamp",
...
par(pp)
```
More details are given in `?plot_residuals`. For the example depth timeseries, these plots demonstrate that out model needs to be interpreted with caution: residuals versus fitted values, temperature and timestamp look reasonable, but the model has not perfectly captured the autocorrelation among residuals and the assumption of normality is clearly violated. 

## Number lines and timelines: `pretty_numline()`
...
```{r, fig.width = 7, fig.height = 5}
pretty_numline(df$timestamp, pch = 21, col = "red", bg = "red", cex = 0.1)
```

This kind of approach is particularly useful for presence or presence/absence observations collected through time. `?pretty_numline()` provides further information and examples, including exemplifying the addition of one dimensional number lines or timelines to existing plots. 

## Histograms and density plots: `pretty_hist()` and `pretty_density()`

Histograms and density plots are also useful components of data exploration and/or modelling diagnostics. Below, I use `pretty_hist()` to create a histogram of the distribution of depths and `pretty_density()` to explore the distribution of observed data in relation to a Gaussian distribution with parameters estimated by the model above: 
...
## Timeseries plot: `Tools4ETS::plot_ts()`

For timeseries data, `Tools4ETS::plot_ts()` links most of the building blocks described above to streamline the creation of complex plots. In the example below, I plot depth against time. I could colour the timeseries by temperature, but I chose to add this as a new axis instead. Axis parameters are controlled by `pretty_axis_args` and `pretty_axis_args_y2` and labels by `mtext_args`. For demonstration purposes, I add diel shading, summary statistics and the predictions of a model. Of course, this is a much more complex plot than desirable, but it's purpose here is solely to demonstrate some of the functionality in `Tools4ETS::plot_ts()`. 

```{r, fig.width = 7, fig.height = 5}
pp <- par(oma = c(2, 2, 2, 2))
Tools4ETS:::plot_ts(x = df$timestamp, 
                    y1 = df$dn, 
                    y2 = df$temp, 
                    pretty_axis_args = list(side = c(3, 2), pretty = list(n = 10), axis = list(las = TRUE)),
                    mtext_args = 
                      list(
                            list(side = 3, text = "Time", line = 2), 
                            list(side = 2, text = "Depth (m)", line = 2), 
                            list(side = 4, text = expression(paste("Temperature (", degree, "C)")), line = 2.5)
                            ), 
                    y2_method = "by_new_axis", 
                    pretty_axis_args_y2 = list(side = 4, pretty = list(n = 10), axis = list(las = TRUE)),
                    add_lines_args_y2 = list(col = "blue"),
                    list_CIs_args = list(pred = p, fadj = function(x){ -x }), 
                    add_model_predictions_args = list(x = dat_flapper_thin$timestamp),
                    summarise_in_bins_args = list(bin = "12 hours", funs = list(mean = mean)),
                    add_lines_args_summaries = list(lwd = 1, col = "red"),
...
`?Tools4ETS::plot_ts()` provides further information and examples. `Tools4ETS::visTS()` is an extension of `plot_ts()` into an interactive environment which facilitates rapid data exploration and the production of publication-quality plots for timeseries data via an R Shiny-Dashboard interface. 
...
`pretty_plot()` can use some other functions, like `stats::qqnorm()`, which is implemented under-the-hood in `plot_residuals()`, as follows:
```{r, fig.width = 7, fig.height = 5}
pp <- par(mfrow = c(1, 2))
qqnorm(m1$std.rsd)
...
`plot.pretty` is an R package designed to make common data exploration tasks and plot production for continuous data easier and more flexible. The package was particularly inspired by the need to visualise and explore ecological timeseries data but most functions have wider applicability. 

`pretty_axis()` is an especially useful function in the creation of publication quality plots in base R. The subsequent addition of lines, colour bars, statistical summaries, shading and model predictions adds value to data exploration and publication-quality plots, but can be surprisingly difficult with base R. `plot.pretty` facilitates these operations. Functions can be used as building blocks or via integrative functions, such as `pretty_plot()`. These operate similarly to functions in base R but harness some of the power of building block functions to create improved plots. 

# Future developments

`pretty.plot` has been designed especially for continuous responses and explanatory variables, although some functions (e.g. `add_shading()`) help with discrete variables or factors. This functionality may be expanded in due course. Suggestions are welcome. However, the principle motivation for `plot.pretty` remains the requirements of (ecological) research, which means that `plot.pretty` does not aspire to provide the level of comprehensive functionality of other plotting packages (e.g. `ggplot2`, `plotly`). 
