...
* `pretty_residuals()` creates prettier diagnostic residual plots (including standard diagnostic plots alongside residuals against covariates, timestamps and the autocorrelation function, if applicable); 
...
* `pretty_ts_mat()` creates 2-dimensional plots of the within and between day variation in a timeseries;
* `pretty_pgram()` creates processed periodogram plots (power spectra); 
* `vis_ts()` is an R Shiny-Dashboard user interface for the interactive exploration of (ecological) timeseries and creation of publication quality plots;
...
* `add_hydrodynamic_ts()` to add hydrodynamic model predictions to (depth) timeseries;
